...
- https://privy.io` };
  ...
  function toPrivySession(user, loginMethod) {
    if (!user) {
      return emptySession();
    }
    const solanaWallet = l5(user);
    const ethereumWallet = l4(user);
  ...
  // src/walletBridge.ts
  function createWalletBridge(env2) {
    let client = null;
  ...
  function normalizePrivyError(error) {
    const privyCode = typeof error === "object" && error !== null && "code" in error && typeof error.code === "string" ? String(error.code) : "";
    const message = typeof error === "object" && error !== null && "error" in error && typeof error.error === "string" ? String(error.error) : error instanceof Error ? error.message : typeof error === "string" ? error : JSON.stringify(error);
    if (privyCode === "oauth_user_denied" || privyCode === "failed_to_complete_login_with_oauth_was_cancelled_by_user") {
      return {
        code: "cancelled",
        message
      };
    }
    if (privyCode === "allowlist_rejected" || privyCode === "invalid_origin" || privyCode === "configuration_error" || privyCode === "pkce_state_code_mismatch") {
      return {
        code: "misconfigured",
        message
      };
    ...
    if (normalizedMessage.includes("origin") || normalizedMessage.includes("redirect") || normalizedMessage.includes("client") || normalizedMessage.includes("appid") || normalizedMessage.includes("app id")) {
      return {
        code: "misconfigured",
        message
      };
    ...
    if (normalizedMessage.includes("network") || normalizedMessage.includes("timeout") || normalizedMessage.includes("unavailable")) {
      return {
        code: "unavailable",
        message
      };
  ...
  var env = window.__glideEnv ?? getDefaultEnv();
